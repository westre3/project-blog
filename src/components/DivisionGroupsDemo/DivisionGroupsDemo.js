    ...
    <LayoutGroup>
      <Card as='section' className={styles.wrapper}>
        <header className={styles.header}>
          <SliderControl label='Number of Groups' className={styles.slider} step={1} min={1} max={4} value={numOfGroups} onChange={ev => setNumOfGroups(Number(ev.target.value))} />
        </header>
            ...
            {range(numOfGroups).map(groupIndex => (
              <div key={groupIndex} className={styles.group}>
                {range(numOfItemsPerGroup).map(index => {
                  const layoutIndex = `${id}-${groupIndex * numOfItemsPerGroup + index}`;
                  return <motion.div layoutId={layoutIndex} key={layoutIndex} className={styles.item} />;
                })}
              </div>
            ))}
          </div>
        ...
        {includeRemainderArea && (
          <div className={styles.remainderArea}>
            ...
            {range(remainder).map(index => {
              const layoutIndex = `${id}-${numOfItems - index - 1}`;
              return <motion.div layoutId={layoutIndex} key={layoutIndex} className={styles.item} />;
            })}
          </div>
        )}
